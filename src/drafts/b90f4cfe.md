...
The Riemann zeta function is a mathematical function whose zeros, the inputs where its value equals zero, help describe patterns in the distribution of prime numbers. Its nontrivial zeros are all its zeros except the readily classified trivial zeros at negative even integers. They occur in the critical strip, the region of the mathematical plane between the vertical lines with real parts zero and one.

The Riemann hypothesis, proposed by German mathematician Bernhard Riemann in 1859, says every nontrivial zero lies on the vertical line halfway through that strip, called the critical line. Nobody has proved that hypothesis, and Claude did not prove it.

The [current Claude preprint, dated August 11, 2026](https://www-cdn.anthropic.com/95c246936988e43127bc6b2ceb7077c1dad2d68e.pdf), is a research paper released before conventional journal peer review. It establishes a lower bound, meaning a proven minimum: at least two thirds of the nontrivial zeros are simple and lie on the critical line. A simple zero has multiplicity one. In the total against which that share is measured, a repeated zero is counted once for each repetition. The claim concerns the share as the number of zeros grows without limit, rather than a finite census, and it does not classify the remaining third.
...
The control mechanics should be equally plain. Permissions prevent the delivery team from changing a campaign from paused to renewed on its own. An append-only event record, a log that accepts new events without altering earlier entries, does something different: it preserves the requested transition, the evidence presented, the authorized decision, and the acceptance-test version so the choice can be audited later.
