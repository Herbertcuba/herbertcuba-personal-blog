...
The senior sponsor should receive one versioned release pack: the workflow boundary, accountable owner, evidence trend, funded exception capacity, residual risk, and an explicit recommendation to expand, hold, or stop. That makes the decision forwardable without hiding the operating detail in several committee papers.
...
Before a portfolio team proposes shadow mode, where an agent recommends actions while a person still executes them, it should answer five questions:
...
For the board, the executive decision rule is short: do not change the org chart until the workflow beats the old process on quality, serious exceptions, owner load, escalation time, and full cost, while remaining safe to reverse.

The operating plan should fit on one page. It needs the same mandatory fields at any scale: workflow boundary and forbidden actions; accountable owner and required approvers; allowed tools, data, and state transitions; baseline and pass measures; funded review and exception capacity; cost ceiling; and authority to expand, hold, or stop. The following support trial is illustrative rather than a universal benchmark. It starts with six weeks in shadow mode.
