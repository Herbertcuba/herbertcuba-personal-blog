...
For that shift to work, system activity must be visible, each area of work needs a capable owner who sets the standard and decides exceptions, decision rights must be explicit, and high-risk cases must reach a person. A leader who removes managers before building those conditions creates an accountability gap. As agents absorb routine handoffs, decisions that once traveled upward need capable owners closer to the work, with clear limits on their authority.
...
Simkin proposes a clean-slate exercise: *"If I were building this business today, from scratch, knowing what AI can do, would it look like this?"* Applied to one specific workflow, the question can expose approval steps and product commitments that survive through habit. Redesigning that workflow first reduces the number of goals, handoffs, and exceptions the new system must reconcile, which lowers the practical cost of rebuilding it.
