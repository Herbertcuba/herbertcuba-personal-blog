...
Simkin argues that the advantage compounds, meaning each cycle can improve the next, because leaner operations can fund better systems and each run adds useful feedback. I think that compounding comes from reusable instructions, sharper standards for judging output, and a history of exceptions. The exception history identifies cases where human review prevents losses greater than its time or expense. Weak feedback loops compound mistakes with equal efficiency. A single productivity percentage would hide differences in how often tasks change, how well quality can be tested, and how costly errors are.
