...
<div class="chapter-tldr"><span class="chapter-tldr__label">In short</span><p>When an agent can regenerate the implementation from a clear spec, the code becomes disposable output and the spec becomes the durable asset.</p></div>
...
One boundary: this applies cleanly to systems you control and can regenerate. For vendor or COTS systems with no agent-accessible code path, the spec becomes documentation of your integration and configuration choices — still valuable, different in kind.
...
<div class="chapter-tldr"><span class="chapter-tldr__label">In short</span><p>Match rigor to the work — but zoom out: this spec-to-execution loop is agnostic to code, marketing, or content, and transforming an org to run it means leaning into Tier 3, not one-person armies. It's the operating model behind APEX.</p></div>
...
None of these is the "right" one, and Tier 3 is not the goal. A weekend prototype run at Tier 3 is absurd; a core banking system run at Tier 1 is negligent. The skill is matching the tier to what you're actually building — and being willing to move up as the stakes rise, or down when they don't.

Zoom out, though, because none of this is really about code. Everything I've described is agnostic to the kind of digital work — the same spec-to-execution loop applies to a marketing campaign, a content library, a data pipeline, a product surface. The layers change name, not shape: business context still constrains strategy, strategy still constrains the artifact, and an agent still generates the artifact from a spec sharp enough to test. I think this is simply how digital production works in the near future — humans and agents each doing what they're best at. Humans hold the strategy: the intent, the constraints, the taste, the trade-offs. Agents hold the execution: fast, tireless, regenerable. The spec is the contract between the two.

And that reframes the tiers as more than a per-project dial. Transforming a whole organization to run this way — specs as the source of truth, agents doing the execution — means deliberately leaning into Tier 3 and getting the one-person armies out of the critical path. The solo build is intoxicating precisely because it's so fast, but an organization made of black boxes understood by one person each is not a transformation; it's a collection of key-person risks waiting to walk out the door. Scaling this is a governance move, not a tooling one: shared ownership, versioned specs, review gates, the boring rigor that lets many people build on the same source of truth.

That organizational shift is the harder, more valuable half of the problem, and it's the one I've spent real time on. I've been developing an operating model — [APEX](/apex/) — for exactly this: how a team designs, specs, and verifies while agents execute and iterate, with the ownership and rigor that make it hold at scale. If this piece resonated, that's the natural next step in the same argument.
...
For a regulated enterprise there's one more question, and a risk committee asks it first: what's the blast radius when someone regenerates code from a spec that's wrong? The failure mode is real — a stale spec can quietly revert a hard-won security decision — but it's governable with controls we already have. Regeneration that touches regulated systems goes through the normal change-approval gate, and what the gate inspects is the spec diff: which acceptance criteria changed, who changed them, sign-off on anything touching a regulated boundary, plus the test run proving the unchanged criteria still hold. That maps cleanly onto controls auditors already recognize — spec-diff-plus-sign-off is change management, the SOX, ISO 27001, and DORA kind, and the test gate is your evidence of control effectiveness. The risk doesn't disappear; it moves somewhere you can see it and put a control on it, instead of living unexamined in the gap between what the docs say and what the code does.
...
<div class="chapter-tldr"><span class="chapter-tldr__label">In short</span><p>Teams that make the shift get fewer rollbacks, shorter cycle time, and — the real prize — a defensible moat: the accumulated, well-maintained clarity about what the system should do and why.</p></div>

A team of eight engineers at a mid-size SaaS company made this shift over three months. Same headcount. Before: the spec was a Jira ticket description nobody reread, and success was measured in code shipped. After spec-first: spec review became part of the definition of done, each feature spec was reviewed before generation, versioned alongside the code, and covered by tests derived from acceptance criteria. Three months in, feature rollback incidents dropped from four per quarter to one. Cycle time from spec to shipped dropped 30%. The change that surprised the team wasn't the speed — it was the clarity. Arguments about requirements dried up because the spec was the argument, already resolved and already versioned.
...
<div class="chapter-tldr"><span class="chapter-tldr__label">In short</span><p>Scarcity moved upstream from code to the definition. The organizations that win will be the ones that started treating their specs like the source code they'd become.</p></div>
