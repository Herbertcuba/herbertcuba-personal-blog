...
The coffee-brand example is greenfield. The harder question is whether the same structure holds when you're working inside an existing enterprise system — one with compliance requirements, legacy constraints, and a risk committee that needs to sign off before anything touches production.

It does, and the mechanics are mostly the same. A mid-size Nordic financial services company — composite, but drawn from real practice — used the stack to rebuild an internal claims-processing workflow. Layer 1 defined the business constraint: reduce average claim resolution from 14 days to under 5, while keeping false-positive rejection rates below 2%. Layer 3's out-of-scope list explicitly excluded any automated denial decision — compliance required human sign-off on every rejection. That one line prevented three months of building the wrong thing.

When regulators later requested evidence that the denial logic hadn't changed between two releases, the team pulled the spec diff and showed exactly which acceptance criterion had been amended and by whom, alongside the test run proving everything else still held. An auditor can work with that. An auditor cannot work with a diff of 40,000 lines of claims-processing logic.

The spec-first approach doesn't disappear under enterprise constraints. It becomes more useful — because the constraints are precisely the things that belong in the spec, making them visible, versioned, and auditable instead of buried in code comments or tribal knowledge.

One important boundary: this applies cleanly to systems you control and can regenerate. For vendor-supplied or COTS systems with no agent-accessible code path, the spec becomes documentation of your integration and configuration choices — still valuable, different in kind.
