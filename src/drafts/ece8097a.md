...
<p class="scifi__film">Star Trek — the transporter</p>
<p>Think of the transporter on the <em>Enterprise</em>. What it actually protects isn't the crew member standing on the pad — it's the <em>pattern</em> in the buffer, the complete specification of every atom. The body is dematerialized and rebuilt from that pattern on the far end; the matter is disposable, the pattern is everything. Lose the person and you rematerialize them from the pattern. Corrupt the pattern, and you get back something subtly, dangerously wrong — which is exactly the plot of every transporter-accident episode, and exactly what spec drift is.</p>
<p>That's the inversion in one image. The spec is the pattern buffer. The code is just this cycle's rematerialization. And it's worth saying early: this isn't a metaphor that only works for software — anything you can specify precisely enough to regenerate, from a campaign to a document to a workflow, sits on the same pad.</p>
</div>
