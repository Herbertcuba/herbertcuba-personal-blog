...
| 2026 | A2A, MCP, WebMCP, TOON, headless CMS | Agent protocols, structured data, connectivity layers | ??? |
...
| Protocol | What it connects | Think of it as |
|----------|-----------------|----------------|
| **WebMCP** | Agent ↔ websites | The browser-native bridge |
| **A2A** | Agent ↔ agent (across organizations) | The inter-agency phone network |
| **MCP** | Agent ↔ tools and data | The universal adapter |
| **TOON** | Data ↔ LLM consumption | The compression codec |

Let me walk through each one.

## WebMCP: the HTTP of the agent web

This is the one that made me sit up straight. WebMCP landed in Chrome 146 Canary earlier this year, a joint effort between Google and Microsoft heading toward W3C standardization.
...
That's what Google's Agent-to-Agent Protocol (A2A) is built for. And the coalition behind it tells you how seriously the industry is taking this problem: Google Cloud plus over 50 partners including Salesforce, SAP, Atlassian, MongoDB, and others. This isn't a side project. It's a coordinated bet on interoperability.
...
The gap A2A fills is significant. Without it, every cross-system agent integration is a custom, point-to-point connection. I've seen this movie before in the API era. Before REST became a shared convention, every integration was bespoke. Every partner required custom work. Scaling was painful. A2A is aiming to prevent that same fragmentation from happening in the agent era.

## MCP: the universal adapter

Anthropic's Model Context Protocol has quickly become the standard for how AI agents connect to tools and data sources. It's the universal adapter between models and the systems they need to interact with: databases, APIs, file systems, third-party services.

What I find worth paying attention to is how MCP, A2A, and WebMCP together form a complete connectivity layer. Each one covers a distinct relationship:

- **WebMCP** handles agent-to-website through the browser.
- **A2A** handles agent-to-agent across organizational boundaries.
- **MCP** handles agent-to-tool-and-data within the agent's own ecosystem.
